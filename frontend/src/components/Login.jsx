...
const Login = ({ setToken = () => {} }) => {
  ...
  const triggerLoginFlow = async (loginEmail, loginPassword) => {
    ...
    try {
      ...
      // Check default admin with simple credentials
      if ((cleanEmail === 'admin' || cleanEmail === 'admin@sentinel.ai') && (loginPassword === 'admin123' || loginPassword === 'Admin@123')) {
        authenticatedUser = { username: 'admin', fullname: 'Admin User', role: 'Administrator', org: 'Sentinel Security Core' };
      } else if (defaultAccounts[cleanEmail] && defaultAccounts[cleanEmail].password === loginPassword) {
